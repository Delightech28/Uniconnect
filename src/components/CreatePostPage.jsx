  ...
  const insertLinkOrMedia = async (type) => {
    ...
    input.onchange = async (e) => {
      ...
      if (file.type.startsWith("video/")) {
        toast.error(
          "Video uploads are not allowed. Please upload images only.",
        );
        return;
      }
      ...
      try {
        ...
        uploadTask.on(
          ...
          async (err) => {
            ...
            toast.error(
              "Upload failed — you can paste a public image URL instead.",
            );
            // fallback: prompt user to paste a public URL
            const fallback = window.prompt(
              "Upload failed. Paste a public image URL (or Cancel to abort)",
            );
            if (fallback) {
          ...
          async () => {
            try {
              ...
              toast.error(
                "Upload succeeded but failed to retrieve URL. Paste a public image URL instead.",
              );
              const fallback = window.prompt(
                "Paste a public image URL (or Cancel to abort)",
              );
              if (fallback) {
                replaceSelection(`![${selected}](${fallback})`);
              }
        ...
        toast.error(
          "Media upload failed — you can paste a public image URL instead.",
        );
        const fallback = window.prompt(
          "Upload failed. Paste a public image URL (or Cancel to abort)",
        );
      ...
      {buttons.map((btn, index) => {
        if (btn.type === "divider") {
          return (
            <div
              key={`divider-${index}`}
              className="h-5 w-px bg-border-light dark:bg-border-dark mx-1"
            ></div>
          );
        }
        return (
          <button
            ...
            type="button"
            className="flex items-center justify-center p-2 rounded hover:bg-primary-light dark:hover:bg-primary/30"
          >
...
const TagInput = ({ tags, setTags }) => {
  ...
  return (
    <div className="space-y-3">
      <p
        className="text-text-light dark:text-text-dark text-base 
font-medium leading-normal"
      >
        Add Tags (up to {5 - tags.length}
        remaining)
      </p>
      <div
        className="flex items-center gap-2 flex-wrap p-3 border 
...
pl-3 pr-2"
          >
            <p
              className="text-primary dark:text-green-200 text-sm 
font-medium leading-normal"
            >
              {tag}
            </p>
...
text-primary dark:text-green-200 text-sm"
              >
                close
              </span>
            </button>
      ...
      if (isEditMode) {
          ...
          try {
            ...
            if (uDoc.exists()) {
              ...
              author.name =
                ...
                "User";
              author.avatarUrl =
                d.avatarUrl || getDefaultAvatar(d.gender || "male");
            } else {
        ...
        if (author.id) {
          try {
            ...
            console.warn(
              "Failed to send post notification to author:",
              notifErr,
            );
          }
        }
        ...
        try {
          await notifyAllUsersPostCreated(
            { id: postRef.id, title: post.title },
            author.id,
          );
        } catch (broadcastErr) {
            ...
            <div className="space-y-6">
              <div>
                <label
                  htmlFor="title"
                  className="text-base 
font-medium leading-normal pb-2 block"
                >
                  Post Title
                </label>
                <input
...
text-base"
              ...
              <BackgroundSelector
                ...
                onBackgroundChange={(bg) =>
                  setPost((prev) => ({ ...prev, background: bg }))
                }
              />
              ...
              >
                <p
                  className="text-slate-500 dark:text-slate-400 
                                text-sm"
                >
                  Status: <span className="font-medium">{status.message}</span>.
                  Last saved at {status.time}
                </p>
