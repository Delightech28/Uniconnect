...
const PodcastSection = ({ docText, topics, setLoading, setLoadingMessage, isPaused }) => {
  ...
  const createPodcast = async () => {
    console.log('[PodcastSection] createPodcast', { selectedTopic, hostCount, duration });
  ...
  const togglePlay = async () => {
    if (isPlaying) {
      ...
      // Use Web Speech API to synthesize and speak the transcript
      const utterance = new SpeechSynthesisUtterance(activePodcast.transcript);
      utterance.rate = 0.95;
      utterance.pitch = 1;
      utterance.volume = 1;
          ...
          <div className="flex-1 bg-white dark:bg-zinc-900 rounded-[40px] sm:rounded-[60px] p-8 sm:p-16 shadow-2xl border border-gray-100 dark:border-zinc-800 flex flex-col justify-center text-center space-y-10">
            ...
            <div className="space-y-6 sm:space-y-8 px-2 sm:px-6">
              ...
              <div className="flex items-center justify-center gap-6 sm:gap-10">
                <button onClick={() => { window.speechSynthesis.cancel(); setIsPlaying(false); }} className="text-gray-500 dark:text-gray-300 active:text-[#07bc0c] transition-colors"><RotateCcw size={28} /></button>
                ...
                <button onClick={togglePlay} className="text-gray-500 dark:text-gray-300 active:text-[#07bc0c] transition-colors"><RotateCw size={28} /></button>
              </div>
            </div>
          </div>
