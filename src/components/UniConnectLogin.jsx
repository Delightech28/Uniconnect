  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (code === "auth/popup-closed-by-user") {
        ...
        setErrorMessage(
          "Unauthorized domain. Add this origin to Firebase Auth authorized domains.",
        );
      } else if (code === "auth/operation-not-allowed") {
        console.error("Google sign-in not enabled:", error);
        toast.error(`Sign-in not allowed: ${message}`);
        setErrorMessage(
          "Google sign-in is not enabled in Firebase Auth settings.",
        );
      } else {
                ...
                <div className="absolute left-4 right-4 bottom-6 flex justify-center">
                  <div className="bg-black/40 backdrop-blur-sm rounded-md px-4 py-3 text-white max-w-[90%]">
                    ...
                    <p className="text-xs md:text-sm mt-1">
                      Sign in to continue to your UniSpace account — your campus
                      hub for study, swap and social.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="w-full md:w-1/2 flex items-center justify-center p-6 pt-24">
            <div className="flex flex-col max-w-[480px] w-full justify-center py-10">
              ...
              <form
                ...
                className="w-full flex flex-col gap-4 px-4 py-3"
              >
