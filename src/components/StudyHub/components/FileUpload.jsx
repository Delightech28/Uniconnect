...
/**
 * FileUpload component for uploading PDF documents
 * @param {Object} props
 * @param {Function} props.onFileUpload - Callback when file is uploaded
 * @param {boolean} [props.isDarkMode] - Dark mode toggle
 * @param {number} [props.progress=0] - Upload progress percentage
 * @param {string} [props.stage] - Current processing stage text
 */
const FileUpload = ({ onFileUpload, isDarkMode, progress = 0, stage }) => {
  ...
  return (
    <div className="h-full w-full flex items-center justify-center transition-colors duration-300 overflow-y-auto bg-white dark:bg-secondary">
        ...
        <div className="flex flex-col items-center text-center space-y-8 md:space-y-12">
          ...
          <div className="flex flex-col items-center space-y-4 md:space-y-6 animate-fade-in">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <h1 className="text-4xl md:text-7xl font-extrabold tracking-tighter text-secondary dark:text-white">
                Study<span className="text-primary">Hub</span>
              </h1>
            </div>
            <p className="text-lg md:text-2xl font-medium leading-relaxed max-w-2xl px-4 text-slate-500 dark:text-slate-400">
          ...
          <div className="relative group w-full max-w-2xl px-4 md:px-0">
            ...
            <div 
              className="relative z-10 p-6 md:p-12 rounded-[2rem] md:rounded-[3.5rem] border shadow-2xl transition-all duration-500 bg-white dark:bg-secondary border-slate-100 dark:border-slate-700 backdrop-blur-xl hover:shadow-xl"
            >
              <div
                className={`w-full min-h-[300px] md:min-h-[400px] rounded-[1.5rem] md:rounded-[2.5rem] flex flex-col items-center justify-center transition-all duration-300 relative overflow-hidden ${
                  isDragging
                    ...
                    : 'border-2 border-dashed border-slate-300 dark:border-slate-600 hover:border-primary hover:bg-background-light dark:hover:bg-slate-700'
                  ...
                  <div className="flex flex-col items-center justify-center text-center p-6 space-y-6 md:space-y-8 w-full">
                    <div className="relative flex items-center justify-center">
                      <svg className="w-32 h-32 md:w-40 md:h-40 transform -rotate-90">
                        <circle cx="50%" cy="50%" r="60" stroke={isDarkMode ? "#1e293b" : "#f1f5f9"} strokeWidth="8" fill="transparent" />
                        <circle cx="50%" cy="50%" r="60" stroke="#07bc0c" strokeWidth="8" strokeDasharray="440" strokeDashoffset={440 - (440 * progress) / 100} strokeLinecap="round" fill="transparent" className="transition-all duration-300 ease-out" />
                      </svg>
