...
const parseJsonResponse = (text) => {
  ...
  try {
    ...
    const startIndex =
      start !== -1 && (startBrace === -1 || start < startBrace)
        ? start
        : startBrace;
...
const getAccentInstruction = (accent) => {
  switch (accent) {
    case "NG":
      return "using a natural Nigerian English style, with clear Nigerian rhythm and local nuances.";
    case "UK":
      return "using a natural British English (UK) style with appropriate British vocabulary.";
    case "US":
      return "using a standard American English (US) style.";
    default:
      return "using a clear neutral English style.";
  }
};
...
const getToneInstruction = (tone) => {
  switch ((tone || "").toUpperCase()) {
    case "FUNNY":
      return "Be highly entertaining and witty. Use educational jokes and keep the energy high. Keep responses conversational and engaging.";
    case "PROFESSIONAL":
      return "Be formal, objective, and precise. Use professional academic terminology while remaining clear and accessible.";
    case "TEACHER":
      return "Be warm, encouraging, and approachable. Explain complex ideas with simple analogies. Use a conversational tone like a friend helping you learn.";
    case "FRIEND":
      return "Be casual and supportive. Talk like a friendly study buddy.";
    default:
      return "Be clear and helpful.";
  }
};
...
const callCloudFunction = async (requestBody, signal) => {
  try {
    console.log(
      "[callCloudFunction] Calling Cloud Function at:",
      CLOUD_FUNCTION_URL,
    );
    ...
    if (!response.ok) {
      ...
      console.error(
        "[callCloudFunction] Cloud Function error:",
        response.status,
        errorText,
      );
      throw new Error(
        `Cloud Function error: ${response.status} - ${errorText}`,
      );
    }
...
const callGenerate = async (modelName, body, signal) => {
  ...
  console.log(
    "[callGenerate] Sending to Cloud Function with systemInstruction:",
...
export const generateTopics = async (text, signal) => {
  console.log(
    "[generateTopics] Starting extraction with text length:",
    text?.length,
  );
  ...
  try {
    ...
    const systemInstruction = `You are a topic extraction assistant. Extract topics from documents and return them as a JSON array. Always respond with valid JSON.`;

    console.log("[generateTopics] Sending request to streamGeminiWithText...");
    ...
    const response = await fetch(CLOUD_FUNCTION_URL, {
      ...
      body: JSON.stringify({
        text: prompt,
        question: "Extract topics from this document",
    ...
    console.log("[generateTopics] Extracted text:", fullText.substring(0, 200));

    let topics = parseJsonResponse(fullText);
 ...
 */
export const initializeChatWithContext = async (
  docText,
  topics,
  tone = "TEACHER",
) => {
...
export const speakText = async (
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      const selectedVoice = findVoiceByLangAndGender(
        voices,
        preferredLangs,
        gender,
        [],
...
export const getQuizFeedback = async (docText, questions, results, signal) => {
  ...
  try {
    ...
    const systemInstruction = `Analyze these quiz results and provide constructive feedback in exactly this JSON format:
...
Return ONLY this valid JSON: { "performanceSummary": "...", "strengths": "...", "weaknesses": "...", "nextSteps": "..." }`;
    ...
    const response = await fetch(
      `${FIREBASE_FUNCTIONS_URL}/streamGeminiWithText`,
      {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          text: docText.substring(0, 2000),
          question: "Analyze quiz results",
          systemInstruction: systemInstruction,
        }),
        signal,
      },
    ...
    const feedback = parseJsonResponse(fullText);
    return (
      feedback || {
        performanceSummary: "Assessment complete.",
        strengths: "Good effort on the quiz.",
        weaknesses: "Review challenging concepts.",
        nextSteps: "Try another quiz to reinforce learning.",
      }
    );
  } catch (error) {
 ...
 */
export const generatePodcastContent = async (
  docText,
  settings = {},
  signal,
) => {
  ...
  try {
    ...
    const hostNames =
      hosts.length > 0
        ? hosts.map((h) => h.name || "Host").join(" and ")
        : "Alex";
    ...
    const systemInstruction = `Create a detailed podcast script from this document with proper host management.
...
RETURN ONLY VALID JSON (no markdown, no code blocks, just raw JSON):
{
  ...
  "segments": [
    ...
    console.log(
      "[generatePodcastContent] Prompt length:",
    ...
    const response = await fetch(
      `${FIREBASE_FUNCTIONS_URL}/streamGeminiWithText`,
      {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          text: docText.substring(0, 3000),
          question: "Create a podcast script",
          systemInstruction: systemInstruction,
        }),
        signal,
      },
    ...
    console.log(
      "[generatePodcastContent] Extracted text:",
      fullText.substring(0, 300),
    );

    const podcastData = parseJsonResponse(fullText);
    console.log(
      "[generatePodcastContent] Parsed podcast data:",
      JSON.stringify(podcastData).substring(0, 200),
    );
    ...
    if (
      !podcastData.segments ||
      !Array.isArray(podcastData.segments) ||
      podcastData.segments.length === 0
    ) {
      console.warn(
        "[generatePodcastContent] No valid segments found in response",
      );
      return {
        audio: "",
        segments: [],
    ...
    console.log(
      "[generatePodcastContent] Successfully generated podcast with",
      podcastData.segments.length,
      "segments",
    );
    return {
      audio: "",
...
export const analyzeDocument = async (docText, signal) => {
  ...
  try {
    const systemInstruction = `Analyze this document and provide:
...
Return ONLY valid JSON: { "summary": "...", "keyPoints": ["p1"], "topics": ["t1"] }`;
    ...
    const analysis = parseJsonResponse(fullText);
    return (
      analysis || {
...
export const askTutor = async (
  ...
  return executeWithRetry(async () => {
    try {
      ...
      // Build systemInstruction for the tutor
      ...
      // Combine document text, chat history, and question
      const combinedText = docText
        ? `DOCUMENT CONTENT:\n${docText}\n\nEND DOCUMENT${chatContext}User Question: ${question}`
        : `${chatContext}User Question: ${question}`;

      console.log("[askTutor] Sending request to streamGeminiWithText...");

      // Use the new streamGeminiWithText function
      const response = await fetch(CLOUD_FUNCTION_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      console.error("[askTutor] Error details:", {
 ...
 */
const findVoiceByLangAndGender = (
  voices,
  preferredLangs,
  gender,
  excludeVoices = [],
  ...
  console.log(
    "[findVoiceByLangAndGender] Looking for",
    gender,
    "voice with accents:",
    preferredLangs,
  );
  ...
  for (let lang of preferredLangs) {
    ...
    console.log(
      "[findVoiceByLangAndGender] Available voices for",
      lang,
      ":",
      voicesForLang.map((v) => v.name).join(", "),
    );

    // Prioritize gender-specific voices
    const genderKeywords =
      gender === "male"
        ? ["male", "man", "boy", "david", "james", "mark"]
        : ["female", "woman", "girl", "zira", "susan", "eva", "victoria"];

    // First: gender-specific Neural/Premium voice
    let voice = voicesForLang.find(
      (v) =>
    ...
    if (voice) {
      console.log(
        "[findVoiceByLangAndGender] Found gender-specific premium voice:",
        voice.name,
      );
      return voice;
    }
    ...
    if (voice) {
      console.log(
        "[findVoiceByLangAndGender] Found gender-specific voice:",
        voice.name,
      );
      return voice;
    }
    ...
    if (voice) {
      console.log(
        "[findVoiceByLangAndGender] Found neural/premium voice:",
        voice.name,
      );
      return voice;
    }
 ...
 */
export const getVoiceForAccent = (
  accent = "US",
  name = "Alex",
  hostIndex = 0,
) => {
  ...
  console.log(
    "[getVoiceForAccent] Gender detected - Name:",
    name,
    "Gender:",
    gender,
    "Accent:",
  ...
  console.log(
    "[getVoiceForAccent] Selected voice:",
    result?.name || "default",
    "Lang:",
    result?.lang || "auto",
  );
...
const createMinimalWavBlob = (transcript, hosts, playbackSpeed = 1.0) => {
  ...
  // Combine all chunks
  const totalSize =
    wavHeader.byteLength + pcmData.byteLength + infoChunk.byteLength;
 ...
 */
export const downloadPodcastAsAudio = async (
  transcript,
  ...
  try {
    console.log(
      "[downloadPodcastAsAudio] 1. Starting audio file generation:",
      filename,
      "Speed:",
      playbackSpeed,
    );
    console.log(
      "[downloadPodcastAsAudio] 2. Transcript length:",
      transcript.length,
    );

    const cleanFilename = filename.includes(".wav")
      ? filename
      : `${filename}.wav`;
    console.log(
      "[downloadPodcastAsAudio] 3. Creating WAV file:",
      cleanFilename,
    );

    const wavBlob = createMinimalWavBlob(transcript, hosts, playbackSpeed);
    console.log(
      "[downloadPodcastAsAudio] 4. WAV blob created, size:",
      wavBlob.size,
      "Speed:",
      playbackSpeed,
    );
    ...
    console.log(
      "[downloadPodcastAsAudio] 6. Triggering download:",
      cleanFilename,
    );
    ...
    console.log(
      "[downloadPodcastAsAudio] 7. Download completed successfully! ✅",
    );
  } catch (error) {
...
export const analyzeQuizPerformance = async (
  ...
  return executeWithRetry(async () => {
    const results = questions.map((q, i) => ({
      ...
      userAnswer: q.options[userAnswers[i]] || "None",
    ...
    return {
      performanceSummary: parsed.performanceSummary || "Quiz completed.",
      strengths: parsed.strengths || "Good effort.",
      weaknesses: parsed.weaknesses || "Review content.",
      nextSteps: parsed.nextSteps || "Keep practicing.",
    };
  }, signal);
};
