...
const StudentReferral = () => {
                      ...
                      if (change.type === "added") {
                        ...
                        try {
                          notifyReferralJoined(user.uid, {
                            id: change.doc.id,
                            referreeName:
                              refereeData.displayName ||
                              refereeData.email?.split("@")[0] ||
                              "A new user",
  ...
  const handleWhatsAppShare = () => {
    const text = `Join me on UniSpace! Use this link to sign up and get verified: ${referralLink}`;
    const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(text)}`;
    window.open(whatsappUrl, "_blank");
  };

  const handleTwitterShare = () => {
    const text = `Just joined UniSpace! 🎓 Join me using my referral link and get verified instantly: ${referralLink}`;
          ...
          <div className="max-w-[960px] w-full flex flex-col gap-8 md:gap-12">
            ...
            <div className="flex flex-col gap-3 animate-fade-in-up">
              <h1 className="text-3xl md:text-4xl font-black leading-tight tracking-tight text-slate-900 dark:text-slate-50">
                Invite Friends,{" "}
                <span className="text-[#07bc0c]">Get Featured</span>
              </h1>
              <p className="text-slate-600 dark:text-slate-400 text-base max-w-2xl">
                Share your unique link and get your profile featured on our
                homepage when you successfully invite 10 friends to UniSpace.
              </p>
            </div>
            ...
            {!isLoading && !isVerified && (
              <div className="p-6 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                <p className="text-amber-900 dark:text-amber-200 font-medium">
                  📋 Complete student verification to access the referral
              ...
              <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="flex flex-col gap-6 flex-1">
                  ...
                  <div className="flex flex-wrap gap-4">
                    <button
                      onClick={handleShare}
                      className="flex flex-col items-center gap-2 w-24 cursor-pointer group hover:opacity-80 transition-opacity"
                    >
                    ...
                    <button
                      ...
                      className="flex flex-col items-center gap-2 w-24 cursor-pointer group hover:opacity-80 transition-opacity"
                    >
                      <div className="p-3.5 rounded-full bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 group-hover:bg-green-200 dark:group-hover:bg-green-900/50 transition-colors">
                    ...
                    <button
                      ...
                      className="flex flex-col items-center gap-2 w-24 cursor-pointer group hover:opacity-80 transition-opacity"
                    >
                      <div className="p-3.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 group-hover:bg-blue-200 dark:group-hover:bg-blue-900/50 transition-colors">
                        <Twitter size={24} />
                      </div>
                      <p className="text-sm font-medium text-slate-700 dark:text-slate-300">
                        Twitter/X
                    ...
                    <button
                      ...
                      className="flex flex-col items-center gap-2 w-24 cursor-pointer group hover:opacity-80 transition-opacity"
                    >
                      <div className="p-3.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 group-hover:bg-blue-200 dark:group-hover:bg-blue-900/50 transition-colors">
                        <Facebook size={24} />
                      </div>
                      <p className="text-sm font-medium text-slate-700 dark:text-slate-300">
                        Facebook
                ...
                <div className="lg:w-80 p-6 rounded-xl shadow-sm bg-white dark:bg-[#102210]/50 border border-slate-200 dark:border-slate-800 flex flex-col justify-center gap-4">
                  <div className="flex justify-between items-center">
                    ...
                    <p className="text-[#07bc0c] text-sm font-bold">
                      {referralsCount} / 10
                    </p>
                  </div>
                  ...
                  <div className="w-full rounded-full bg-slate-200 dark:bg-slate-700 h-2.5 overflow-hidden">
                    <div
                      className="bg-[#07bc0c] h-2.5 rounded-full transition-all duration-1000 ease-out"
                      style={{
                        width: `${Math.min(referralsCount * 10, 100)}%`,
                      }}
                    ></div>
                  </div>
                  <p className="text-slate-600 dark:text-slate-400 text-sm">
                    ...
                    <span className="font-bold text-slate-800 dark:text-slate-200">
                      {Math.max(10 - referralsCount, 0)} referrals
                    </span>{" "}
                    away from being featured!
                  </p>
                </div>
              </div>
            )}
