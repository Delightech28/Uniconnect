...
const productData = {
  ...
  description: {
    short:
      "Slightly used HP Elitebook 840 G5 available for sale. In perfect working condition with minimal signs of wear. Excellent for students and professionals.",
    specs: [
...
function ProductDetailsPage() {
            ...
            if (!data.sellerAvatarUrl && data.sellerId) {
              try {
                ...
                if (err && err.code === "permission-denied") {
                  console.warn(
                    "Permission denied when fetching seller profile; using embedded avatar or placeholder",
                  );
                } else {
  ...
  // Set current user and check purchase status
  useEffect(() => {
  ...
  const handleMessageSeller = async () => {
    ...
    try {
      ...
      convSnap.forEach((docSnap) => {
        ...
        if (
          d.participants &&
          d.participants.includes(sellerId) &&
          d.context &&
          d.context.listingId === product.id
        ) {
      ...
      try {
        ...
        if (sellerUserDoc.exists()) {
          ...
          sellerName =
            displayName || emailPrefix || sellerData.email || sellerName;
        ...
        console.warn(
          "Could not fetch seller user info; using defaults:",
          userErr,
        );
      }
  ...
  const handleDownloadPDF = async (pdfIndex) => {
    try {
      const getSecurePDFUrl = httpsCallable(functions, "getSecurePDFUrl");
      const result = await getSecurePDFUrl({
        listingId: product.id,
        pdfIndex,
      });
    ...
    return (
      <div className="bg-background-light dark:bg-background-dark min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-secondary dark:text-white text-lg">
            Loading product...
          </p>
        </div>
      </div>
...
dark:text-slate-300 hover:text-primary dark:text-[#a8d5a8] dark:hover:text-primary dark:hover:text-primary dark:text-[#a8d5a8] dark:hover:text-primary"
                onClick={() => navigate("/unimarket")}
              >
...
dark:text-slate-400"
                  ...
                  <h1
                    className="text-lg sm:text-2xl lg:text-3xl font-bold text-secondary 
...
dark:text-slate-400"
                  ...
                  <div className="mt-6 flex flex-col gap-4">
                    <button
                      onClick={async () => {
                        if (!verifyingLoading && !verified) {
                          if (status === "failed") {
                            toast.error(
                              "Your verification failed. You cannot buy items.",
                            );
                            navigate("/verification-failed");
                          } else {
                        ...
                        if (buyer && product.sellerId) {
                          try {
                            ...
                            toast.success(
                              "Purchase confirmed! Check your notifications.",
                            );
                            navigate("/wallet");
                          } catch (err) {
                    ...
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-lg h-10 sm:h-11 px-4 sm:px-6 bg-slate-200 dark:bg-slate-700 text-secondary dark:text-white text-xs sm:text-sm font-bold leading-normal transition-colors hover:bg-slate-300 dark:hover:bg-slate-600 active:scale-95"
                    >
...
shadow-md p-6 mt-8"
            >
              <h2
                className="text-lg sm:text-xl lg:text-2xl font-bold text-secondary 
dark:text-white"
              >
                Description
              </h2>
...
styling */}
              <div
                className="prose prose-slate dark:prose-invert mt-4 
max-w-none text-slate-600 dark:text-slate-300"
            ...
            {product.pdfUrls && product.pdfUrls.length > 0 && (
              <div className="bg-white dark:bg-secondary rounded-xl shadow-md p-6 mt-8">
                ...
                <div className="mt-4 space-y-2">
                  ...
                  (currentUser && currentUser.uid === product.sellerId) ? (
                    product.pdfUrls.map((pdfUrl, index) => (
                      <button
                        key={index}
                        onClick={() => handleDownloadPDF(index)}
                        className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors w-full text-left"
                      >
                    ...
                    <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                      <p className="text-slate-600 dark:text-slate-400">
                        Purchase this item to access the documents.
                      </p>
                    </div>
                  )}
                </div>
