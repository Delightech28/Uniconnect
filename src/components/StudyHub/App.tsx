...
const App: React.FC = () => {
  ...
  const checkApiKey = useCallback(async () => {
    try {
      ...
      console.warn("API key check failed, continuing in demo mode:", error);
      setHasApiKey(true);
    } finally {
    ...
    return (
      <div className={`h-screen w-full flex items-center justify-center p-6 ${isDarkMode ? 'bg-slate-950' : 'bg-slate-50'}`}>
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-[#07bc0c]/20 border-t-[#07bc0c] rounded-full animate-spin mx-auto mb-4"></div>
          <p className={`text-lg font-bold ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>Initializing StudyHub...</p>
        </div>
      </div>
    );
  }
                ...
                <div className="max-w-2xl w-full flex flex-col items-center my-auto min-h-0 flex-shrink">
                    <div className="relative w-32 h-32 md:w-56 md:h-56 flex items-center justify-center mb-6 shrink-0">
                        <svg className="w-full h-full transform -rotate-90" viewBox="0 0 200 200">
                            <circle cx="100" cy="100" r="90" stroke={isDarkMode ? "#1e293b" : "#f1f5f9"} strokeWidth="10" fill="transparent" />
                            <circle cx="100" cy="100" r="90" stroke="#07bc0c" strokeWidth="10" strokeDasharray="565" strokeDashoffset={565 - (565 * loadingProgress) / 100} strokeLinecap="round" fill="transparent" className="transition-all duration-500 ease-out" />
                        </svg>
