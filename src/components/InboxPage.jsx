  ...
  >
    <div className="flex items-center gap-3 w-full min-w-0">
      ...
      <div className="relative shrink-0">
        ...
        />
        {convo.isOnline && (
          <div className="absolute bottom-0 right-0 size-3 rounded-full bg-green-500 ring-2 ring-white dark:ring-background-dark animate-pulse-soft" />
        )}
      </div>
      ...
      <div className="flex flex-col justify-center min-w-0 flex-1">
        <div className="flex justify-between items-center gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <p
              className={`text-sm truncate transition-all duration-200 ${isActive || convo.unreadCount > 0 ? "font-bold text-text-primary-light dark:text-text-primary-dark" : "font-medium text-text-primary-light dark:text-text-primary-dark"}`}
            >
              {convo.name}
            </p>
            {convo.gender && (
        ...
        <p
          className={`text-xs truncate transition-all duration-200 ${isActive ? "text-text-primary-light dark:text-text-primary-dark font-medium" : "text-text-secondary-light dark:text-text-secondary-dark"}`}
        >
          {convo.lastMessage}
        </p>
      </div>
...
const MessageBubble = ({
  ...
  return (
    <div
      className={`flex items-end gap-2 group ${isMe ? "flex-row-reverse" : ""} animate-slide-up`}
    >
      <div
        onClick={(e) => {
          e.stopPropagation();
        ...
        style={{
          backgroundImage: `url(${isMe ? MY_AVATAR_URL : senderAvatar})`,
        }}
      ...
      />
      <div
        className={`max-w-xs sm:max-w-sm md:max-w-md ${isMe ? "flex flex-col items-end" : ""}`}
      >
        {!isMe && senderName && (
          ...
          {message.gifUrl && (
            <div className="mt-2">
              <img
                src={message.gifUrl}
                alt="GIF"
                className="max-w-xs rounded-lg max-h-64 animate-fade-in"
              />
            </div>
          )}
          {message.fileUrl && (
            <div className="mt-2">
              {message.fileType?.startsWith("image/") ? (
                ...
                <a
                  ...
                  className="flex items-center gap-2 text-xs sm:text-sm underline hover:opacity-75 transition-opacity"
                >
        ...
        <div
          className={`flex items-center gap-1 mt-1 ${isMe ? "flex-row-reverse mr-2" : "ml-2"}`}
        >
          ...
          {isMe && (
            <span
              className={`text-xs ${message.readAt ? "text-blue-500" : "text-gray-400"}`}
...
function InboxPage() {
              ...
              if (otherParticipantId) {
                try {
                  ...
                  if (otherUserDoc.exists()) {
                    ...
                    console.log(
                      "Fetched missing seller info for conversation:",
                      name,
                    ); // Debug
                  }
                } catch (err) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        console.log(
          "Per-user conversations snapshot received, count:",
          snap.size,
        );
        const items = snap.docs.map((d) => {
          ...
          return {
            ...
            otherParticipantId:
              (data.participants || []).find((p) => p !== user.uid) || null,
            name: data.name || "",
  ...
  useEffect(() => {
      ...
      (async () => {
        try {
          ...
          if (convoDoc.exists()) {
            ...
            try {
              await updateDoc(doc(db, "conversations", convoParam), {
                unreadCount: 0,
              });
            } catch (e) {}
            return;
    ...
    (async () => {
      try {
        ...
        try {
          await setDoc(
            doc(db, "users", user.uid, "conversations", convoId),
            convoPayload,
            { merge: true },
          );
          await setDoc(
            doc(db, "users", recipientId, "conversations", convoId),
            convoPayload,
            { merge: true },
          );
        } catch (err) {
        ...
        console.error(
          "Failed to open/create conversation from profile recipientId:",
          err,
        );
      }
    })();
  ...
  useEffect(() => {
    ...
    // Mark messages as read by setting unreadCount to 0
    updateDoc(doc(db, "users", user.uid, "conversations", activeConvoId), {
      unreadCount: 0,
    }).catch((err) =>
      console.warn("Failed to mark conversation as read:", err),
    );
  ...
  const mergedConversations = useMemo(() => {
    ...
    const byId = new Map();
    top.forEach((c) => {
      console.log("Adding top-level conversation:", c.id, {
        name: c.name,
    ...
    mirrors.forEach((m) => {
      console.log("Adding mirrored conversation:", m.id, {
        name: m.name,
    ...
    console.log(
      ...
      combined.length,
      combined.map((c) => ({ id: c.id, name: c.name })),
    );

    // Sort by timestamp if available (fallback to unchanged order)
    combined.sort((a, b) => {
      const ta =
        a.lastTimestamp && a.lastTimestamp.toDate
          ? a.lastTimestamp.toDate().getTime()
          : a.lastTimestamp
            ? a.lastTimestamp
            : 0;
      const tb =
        b.lastTimestamp && b.lastTimestamp.toDate
          ? b.lastTimestamp.toDate().getTime()
          : b.lastTimestamp
            ? b.lastTimestamp
            : 0;
      return (tb || 0) - (ta || 0);
      ...
      (async () => {
        try {
          ...
          try {
            ...
            await setDoc(
              doc(db, "users", otherId, "conversations", convoId),
              convoPayload,
              { merge: true },
            );
          } catch (err) {
            console.warn(
              "Failed to create per-user conversation mirrors:",
              err,
            );
          }
  ...
  const handleSendMessage = async () => {
    if ((!newMessage.trim() && !selectedFile) || !user || !activeConvoId)
      return;
          ...
          reader.onload = async () => {
            try {
              ...
              const convoDoc = await getDoc(
                doc(db, "conversations", activeConvoId),
              );
              if (convoDoc.exists()) {
                ...
                if (recipientId) {
                  try {
                    await notifyNewMessage(recipientId, {
                      ...
                      senderAvatar: null,
                      messagePreview:
                        newMessage.trim().substring(0, 50) || "📎 File sent",
                    });
                  } catch (notifErr) {
  ...
  const totalUnreadCount = useMemo(() => {
    return mergedConversations.reduce(
  ...
  return (
    <div
      className={`flex h-screen w-full flex-col bg-background-light dark:bg-background-dark ${darkMode ? "text-white" : "text-text-primary-light"} font-display`}
    >
      {/* Header */}
      ...
      <main className="flex flex-1 overflow-auto">
        ...
        <aside
          className={`w-full sm:w-80 md:w-96 flex-col border-r border-border-light dark:border-border-dark bg-panel-light dark:bg-panel-dark transition-all duration-300 transform ${
        ...
        >
          ...
          <div className="p-3 sm:p-4 border-b border-border-light dark:border-border-dark">
            ...
            <div className="relative mb-3 rounded-full border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark transition-all duration-300 focus-within:border-green-500 focus-within:border-2 focus-within:shadow-lg focus-within:shadow-green-500/20">
              <span className="material-symbols-outlined text-base absolute left-3 top-1/2 -translate-y-1/2 text-text-secondary-light dark:text-white transition-colors duration-300 pointer-events-none">
                search
              </span>
              <input
              ...
              filteredConversations.map((convo) => (
                <ConversationItem
                  ...
                  onAvatarClick={() =>
                    ...
                    navigate(`/profile/${convo.otherParticipantId}`)
                  }
                />
              ))
            ) : (
        ...
        {/* Main Chat Area */}
        <section
          className={`flex-1 flex flex-col bg-background-light dark:bg-background-dark transition-all duration-300 ${
        ...
        >
          {activeConversation ? (
                ...
                {messages.length === 0 ? (
                  <div className="flex h-full items-center justify-center text-text-secondary-light dark:text-text-secondary-dark animate-fade-in">
                    <div className="text-center">
                      <span className="material-symbols-outlined text-5xl opacity-20 block mb-2">
                        mail
                      </span>
                      <p className="text-sm">
                      ...
                      <div className="flex items-end gap-2 animate-fade-in">
                        ...
                        <div className="max-w-xs md:max-w-md">
                          <div className="p-3 rounded-2xl rounded-bl-none bg-gray-200 dark:bg-gray-700 flex items-center gap-1.5">
                            <span
                              className="size-2 rounded-full bg-text-secondary-light dark:bg-text-secondary-dark animate-bounce"
              ...
              <div className="relative border-t border-border-light dark:border-border-dark bg-panel-light dark:bg-panel-dark p-3 sm:p-4 shrink-0 space-y-2 sm:space-y-3">
                {selectedFile && (
                  <div className="flex items-center gap-2 p-2 bg-background-light dark:bg-background-dark rounded-lg animate-fade-in">
                    ...
                    <span className="text-xs sm:text-sm flex-1 truncate font-medium">
                      {selectedFile.name}
                    </span>
                    <button
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                <span className="material-symbols-outlined text-text-secondary-light dark:text-white text-6xl opacity-20 block mb-4">
                  mail
                </span>
                <h3 className="text-lg sm:text-xl text-text-primary-light dark:text-white font-bold mb-2">
          ...
          <div className="w-72 border-l border-border-light dark:border-border-dark bg-panel-light dark:bg-panel-dark p-4 overflow-y-auto hidden md:block">
            ...
            <div className="text-center mb-6 pb-6 border-b border-border-light dark:border-border-dark">
              ...
              {activeConversation.gender && (
                <GenderBadge gender={activeConversation.gender} />
              )}
              <p
                className={`text-xs mt-2 ${activeConversation.isOnline ? "text-green-500" : "text-text-secondary-light dark:text-text-secondary-dark"}`}
              >
