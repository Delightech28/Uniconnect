...
const UniConnectRegistration = () => {
  ...
  const allInstitutions = Object.entries(universityData).flatMap(([k, list]) =>
    list.map((name) => ({ name, category: k })),
  );
  const filteredInstitutions = allInstitutions.filter((i) => {
    if (
      institutionCategory &&
      institutionCategory !== "all" &&
      i.category !== institutionCategory
    )
      return false;
  ...
  const handleNext = async (e) => {
    ...
    if (step === 2 && formData.registerAs === "guest") {
      ...
      try {
        ...
        const guestUsername = await generateUsername(
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (code === "auth/popup-closed-by-user") {
        ...
        setError(
          "Unauthorized domain. Please add this app origin to Firebase Auth authorized domains.",
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err && err.code === "auth/network-request-failed") {
        setError(
          "Network request failed. Check your internet connection and browser extensions that may block requests (adblocker, privacy plugins).",
        );
        alert(
          "Network error: could not reach Firebase. Check your connection and API key restrictions in the Firebase console.",
        );
      } else if (err && err.code && err.code.includes("api-key")) {
        setError(
          "Invalid or restricted API key. Ensure VITE_FIREBASE_API_KEY is correct and not restricted to the wrong origins.",
        );
        alert(
          "API key error: please verify your Firebase API key and restrictions.",
        );
      } else {
        setError(err.message || "Failed to create account");
        alert(`Error: ${err.message}`);
                ...
                <div className="absolute left-4 right-4 bottom-6 flex justify-center">
                  <div className="bg-black/40 backdrop-blur-sm rounded-md px-4 py-3 text-white max-w-[90%]">
                    <h3 className="text-lg md:text-2xl font-extrabold">
                      UniSpace — Join your campus community
                    </h3>
                    <p className="text-xs md:text-sm mt-1">
                      Find classmates, sell and buy books, and connect across
                      campus. Professional, private, and made for students.
                    </p>
                  </div>
                </div>
              </div>
...
2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                      ...
                      <div className="relative flex items-center">
                        <div
                          className="w-full flex-grow border-t border-slate-300
dark:border-slate-600"
                        ></div>
                        <span
                          className="mx-4 flex-shrink text-sm font-medium
...
text-slate-700 dark:text-slate-300"
                        ...
                        <select
                          className="form-select block w-full rounded-lg
border-slate-300 bg-background-light p-4 text-base text-slate-900
...
text-slate-700 dark:text-slate-300"
                        ...
                        <div className="flex gap-4">
                          <label
                            className="flex items-center gap-2 cursor-pointer flex-1 p-3 border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors"
                            style={{
                              borderColor:
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <button
                              type="button"
                              onClick={() =>
                                setInstitutionOpen(!institutionOpen)
                              }
                              className="block w-full bg-background-light p-4 text-base text-slate-900 focus:border-primary focus:ring-primary dark:bg-slate-800 dark:text-white flex items-center justify-between border border-[#cfdbe7] dark:border-slate-600 rounded-lg"
                            >
                              <span
                                className={`${formData.institution ? "text-slate-900 dark:text-white" : "text-slate-400"}`}
                              >
                            ...
                            {institutionOpen && (
                              <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-white dark:bg-slate-800 border border-[#cfdbe7] dark:border-slate-700 rounded-lg p-3 max-h-64 overflow-auto shadow-lg">
                                <div className="flex gap-3 mb-3 flex-wrap">
                                  <label className="inline-flex items-center gap-2 text-slate-900 dark:text-slate-300">
                                    <input
                                      type="radio"
                                      name="instCat"
                                      value="federal"
                                      checked={
                                        institutionCategory === "federal"
                                      }
                                      onChange={() =>
                                        setInstitutionCategory("federal")
                                      }
                                    />{" "}
                                  ...
                                  <label className="inline-flex items-center gap-2 text-slate-900 dark:text-slate-300">
                                    <input
                                      ...
                                      name="instCat"
                                      value="state"
                                      checked={institutionCategory === "state"}
                                      onChange={() =>
                                        setInstitutionCategory("state")
                                      }
                                    />{" "}
                                  ...
                                  <label className="inline-flex items-center gap-2 text-slate-900 dark:text-slate-300">
                                    <input
                                      ...
                                      onChange={() =>
                                        setInstitutionCategory("private")
                                      }
                                    />{" "}
                                    <span className="text-sm">Private</span>
                                  </label>
                                  <label className="inline-flex items-center gap-2 ml-auto text-slate-900 dark:text-slate-300">
                                    <input
                                      type="radio"
                                      name="instCat"
                                      value="all"
                                      checked={institutionCategory === "all"}
                                      onChange={() =>
                                ...
                                <div className="mb-2">
                                  <input
                                    ...
                                    className="block w-full rounded-md border border-[#cfdbe7] dark:border-slate-600 bg-background-light dark:bg-slate-900 text-slate-900 dark:text-white p-2"
                                  />
                                </div>
                                  ...
                                  {filteredInstitutions.map((inst) => (
                                    <li key={inst.name} className="py-2">
                                      <button
                                        type="button"
                                        onClick={() =>
                                          selectInstitution(inst.name)
                                        }
                                        className={`w-full text-left text-sm p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700 ${formData.institution === inst.name ? "font-bold text-primary" : "text-slate-700 dark:text-slate-200"}`}
                                      >
                                        {inst.name}
                                      </button>
                                    </li>
...
font-bold tracking-wide text-white hover:bg-primary/90"
                      ...
                      <p
                        className="text-text-primary dark:text-gray-300 text-sm
text-center"
                      >
                        I have an account{" "}
                        <Link
...
dark:text-white"
                      >
                        Complete Your Guest Account
                      </h1>
                      <p className="text-slate-600 dark:text-slate-300 mt-2">
                        You're almost done! Click below to create your guest
                        account.
                      </p>
                    </div>
