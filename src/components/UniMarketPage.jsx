...
const ProductCard = ({ product }) => {
      ...
      const fetchSellerAvatar = async () => {
        try {
          ...
          if (err && err.code === "permission-denied") {
            console.warn(
              "Permission denied when fetching seller avatar; using placeholder",
            );
  ...
  return (
    <div
      className="bg-white dark:bg-secondary rounded-xl shadow-md
...
dark:text-slate-400"
        ...
        <h3
          className="font-bold text-lg text-secondary dark:text-white
mt-1 flex-grow"
        >
          {product.name}
        </h3>
...
border-slate-200 dark:border-slate-700"
        >
          <img
            alt={`${product.sellerName}'s profile`}
...
md:justify-between gap-4 mb-8"
            >
              <h1
                className="text-secondary dark:text-white text-xl sm:text-2xl lg:text-3xl
font-bold"
              >
                UniMarket
              </h1>
