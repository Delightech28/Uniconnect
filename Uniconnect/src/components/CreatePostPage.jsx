  ...
  const insertLinkOrMedia = async (type) => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        uploadTask.on(
          ...
          async (err) => {
            ...
            const fallback = window.prompt(
              "Upload failed. Paste a public URL for the media (or Cancel to abort)",
            );
            if (fallback) {
              if (type === "image") {
          ...
          async () => {
            try {
              ...
              toast.error(
                "Upload succeeded but failed to retrieve URL. Paste a public URL instead.",
              );
              const fallback = window.prompt(
                "Paste a public URL for the media (or Cancel to abort)",
              );
              if (fallback) {
                if (type === "image") {
                  replaceSelection(`![${selected}](${fallback})`);
        ...
        toast.error(
          "Media upload failed — you can paste a public URL instead.",
        );
        const fallback = window.prompt(
          "Upload failed. Paste a public URL for the media (or Cancel to abort)",
        );
