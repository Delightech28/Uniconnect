...
const ActionButtons = ({ status, onDelete }) => {
  return (
    <div className="flex justify-end items-center gap-2">
      <button
        className="p-2 text-slate-500 hover:text-primary dark:text-[#a8d5a8] dark:hover:text-primary
...
dark:text-slate-400 dark:hover:text-green-500"
          title="Relist Item"
        >
...
function MyListingsPage() {
  ...
  const formatExpiresIn = (createdAt) => {
    ...
    const daysLeft = Math.max(
      0,
      Math.floor((expiryDate - now) / (24 * 60 * 60 * 1000)),
    );
...
font-display text-secondary dark:text-slate-200 min-h-screen flex flex-col flex-col"
      >
        ...
        <div
          className="relative flex h-auto w-full flex-col group/design-root
overflow-x-hidden"
        >
          <div className="layout-container flex h-full grow flex-col">
            ...
            <main className="flex-1 flex-col px-4 sm:px-6 lg:px-10 py-8">
              <div
                className="layout-content-container flex flex-col max-w-7xl
mx-auto"
              >
                <div
...
text-xl sm:text-2xl lg:text-3xl font-bold leading-tight mb-2"
                    >
...
leading-normal w-full sm:w-auto flex-shrink-0 hover:bg-primary/90 active:scale-95 transition-all"
                    href="/sell-item"
                  >
...
dark:text-slate-400"
                          ...
                          <th
                            className="p-4 text-sm font-semibold text-slate-500
dark:text-slate-400 text-right"
                          >
                            Actions
                          </th>
                        </tr>
...
dark:text-slate-400"
                              >
                                Expires in: {formatExpiresIn(item.createdAt)}
                              </span>
                            </div>
                            <div>
                              <ActionButtons
