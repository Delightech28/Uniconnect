...
const App = () => {
  ...
  // Helper to process PDF files and get cached text
  const processFilesToText = async (files) => {
    const processPDF = httpsCallable(functions, "processPDFOnce");
    ...
    for (const file of files) {
      const base64 = file.data.includes(",")
        ? file.data.split(",")[1]
        : file.data;
      const result = await processPDF({ pdf: base64, userId: user.uid });
  ...
  const handleProcess = async (mode, strategy = 'UNITS') => {
    ...
    if (
      ...
      setProcessingState((prev) => ({
        ...prev,
        error: "Please upload both course material and past questions.",
      }));
      return;
    }
    if (mode === ResultMode.REVIEW && courseFiles.length === 0) {
      setProcessingState((prev) => ({
        ...prev,
        error: "Please upload course material to create FlashDoc.",
      }));
      return;
    }
    ...
    try {
      ...
      // Process files to get cached text
      setStatusMsg("Processing documents...");
      const textA = await processFilesToText(filesA);
      const textB = filesB.length > 0 ? await processFilesToText(filesB) : "";
      ...
      setProcessingState({
        ...
        error:
          err.message ||
          "An unexpected error occurred. Please verify your API key and try again.",
        result: null,
      });
    } finally {
      abortControllerRef.current = null;
  ...
  return (
    <div
      className="min-h-screen bg-gray-50 dark:bg-[#0b0f1a] text-gray-900 dark:text-gray-100 pb-4 md:pb-8 font-sans transition-colors"
      ...
      <main className="max-w-6xl mx-auto px-4 mt-4 md:mt-10">
        ...
        {processingState.isLoading && (
          <div className="fixed inset-0 bg-white/95 dark:bg-[#0b0f1a]/95 backdrop-blur-md z-50 flex flex-col items-center justify-center p-4">
            <div className="relative w-32 h-32 md:w-56 md:h-56 flex items-center justify-center mb-6">
              ...
              <svg className="absolute inset-0 w-full h-full -rotate-90">
                ...
                <circle
                  ...
                  strokeLinecap="round"
                  className="text-primary/40 opacity-80 animate-spin-slow"
                />
              </svg>
            </div>
            <h3
              className={`text-sm md:text-xl font-bold mb-1 text-gray-900 dark:text-white text-center px-4 transition-opacity duration-300 ${msgVisible ? "opacity-100" : "opacity-0"}`}
            >
              {statusMsg}
            </h3>
            ...
            <div className="flex flex-col items-center mt-6">
              <span className="text-primary text-[10px] md:text-sm font-bold mb-4">
                {progress}% Complete
              </span>
              <button
        ...
        {processingState.error && (
          <div className="mb-4 mx-auto max-w-3xl bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 rounded-lg p-3 flex items-start animate-fade-in">
            ...
            <p className="text-red-800 dark:text-red-400 text-xs md:text-sm">
              {processingState.error}
            </p>
          </div>
        )}
        ...
        {processingState.result ? (
            ...
            {isStreaming && (
              <div className="mb-3 bg-white dark:bg-gray-800 rounded-xl p-3 border border-primary/20 flex items-center justify-between shadow-sm animate-fade-in">
                <div className="flex items-center gap-3 overflow-hidden">
                  ...
                  <span className="text-xs md:text-sm font-bold text-gray-700 dark:text-gray-300 truncate tracking-tight">
                    {statusMsg}
                  </span>
          ...
          <div
            className={`space-y-4 md:space-y-6 ${!processingState.isLoading ? "animate-fade-in" : "opacity-0"}`}
          >
            {activeView === "summary" ? (
              <div className="max-w-xl mx-auto flex flex-col gap-1.5 md:gap-2">
                <div className="min-h-[120px] md:min-h-[220px]">
                  <FileUpload
                    ...
                    onFileRemove={(id) =>
                      setSummaryFiles((prev) => prev.filter((f) => f.id !== id))
                    }
